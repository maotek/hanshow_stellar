...
// Large 38-page working canvas; unlike state variables it does not need retention
// across deep sleep and keeping it in BSS avoids consuming the flash prefix.
uint8_t epd_buffer[epd_buffer_size];
uint8_t *epd_temp = epd_buffer; // share the 15 KB framebuffer on the 8258
...
{
    // Conversion is performed a scan line at a time by the EPD driver. Keeping
    // one framebuffer is necessary because two 400x300 planes exceed retention RAM.
    (void)pSrc;
    (void)pDst;
    (void)width;
    (void)height;
...
{
    // The G4 decoder's 13 KB workspace cannot coexist safely with a 15 KB
    // 400x300 framebuffer on the TLSR8258. Raw BLE framebuffer uploads remain
    // supported; reject TIFF locally instead of risking stack corruption.
    (void)pData;
    (void)iSize;
}
...
static void draw_watch_digit(OBDISP *display, int x, int y, uint8_t digit)
{
    ...
    if (mask & 0x01) obdRectangle(display, x + 10, y,       x + 54, y + 11, 1, 1);
    if (mask & 0x02) obdRectangle(display, x + 56, y + 10,  x + 67, y + 59, 1, 1);
    if (mask & 0x04) obdRectangle(display, x + 56, y + 72,  x + 67, y + 121, 1, 1);
    if (mask & 0x08) obdRectangle(display, x + 10, y + 120, x + 54, y + 131, 1, 1);
    if (mask & 0x10) obdRectangle(display, x,      y + 72,  x + 11, y + 121, 1, 1);
    if (mask & 0x20) obdRectangle(display, x,      y + 10,  x + 11, y + 59, 1, 1);
    if (mask & 0x40) obdRectangle(display, x + 10, y + 60,  x + 54, y + 71, 1, 1);
}
...
_attribute_ram_code_ void epd_display(struct date_time _time, uint16_t battery_mv, int16_t temperature, uint8_t full_or_partial)
{
    ...
    obdWriteStringCustom(&obd, (GFXfont *)&Dialog_plain_16,
                         116, 25, (char *)buff, 1);
    obdRectangle(&obd, 24, 38, 375, 40, 1, 1);

    // Large custom seven-segment numerals use most of the square display.
    draw_watch_digit(&obd, 31, 55, _time.tm_hour / 10);
    draw_watch_digit(&obd, 107, 55, _time.tm_hour % 10);
    obdRectangle(&obd, 191, 91, 201, 101, 1, 1);
    obdRectangle(&obd, 191, 137, 201, 147, 1, 1);
    draw_watch_digit(&obd, 218, 55, _time.tm_min / 10);
    draw_watch_digit(&obd, 294, 55, _time.tm_min % 10);

    // A restrained status row keeps useful information without crowding.
    obdRectangle(&obd, 24, 205, 375, 207, 1, 1);
    obdRectangle(&obd, 43, 229, 96, 250, 1, 0);
    obdRectangle(&obd, 38, 235, 43, 244, 1, 1);
    battery_width = (battery_level * 48) / 100;
    if (battery_width)
        obdRectangle(&obd, 46, 232, 45 + battery_width, 247, 1, 1);
