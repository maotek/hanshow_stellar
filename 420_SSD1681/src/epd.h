...
// OneBitDisplay packs pixels in vertical groups of eight. The final four
// rows therefore need a complete byte page even though the panel transfer
// itself is exactly (400 / 8) * 300 bytes.
#define epd_buffer_size (epd_width * ((epd_height + 7) / 8))
#define epd_transfer_size ((epd_width / 8) * epd_height)
