...
_attribute_ram_code_ uint16_t get_battery_mv(void)
{
	...
	// PB7 is connected to the midpoint of a 1 MOhm / 1 MOhm divider.
	// adc_base_init() leaves the GPIO output disabled so it does not disturb
	// the divider. adc_vbat_init() must not be used here: that SDK function
	// drives the selected GPIO high.
    adc_init();
	adc_base_init(GPIO_PB7);
    adc_power_on_sar_adc(1);
	divider_mv = adc_sample_and_get_result();
	adc_power_on_sar_adc(0);

	return (uint16_t)(divider_mv * 2);
}

_attribute_ram_code_ uint8_t get_battery_level(uint16_t battery_mv)
{
	// Project battery gauge: 3.3 V empty, 4.0 V full.
	if (battery_mv <= 3300)
		return 0;
	if (battery_mv >= 4000)
		return 100;

	return (uint8_t)(((uint32_t)(battery_mv - 3300) * 100) / 700);
}
