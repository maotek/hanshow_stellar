...
_attribute_ram_code_ void init_led(void)
{
    ...
    /* led_mask lives in retention RAM. Reapply it after every GPIO init so a
       deep-retention wake does not turn an active LED off permanently. */
...
_attribute_ram_code_ void set_led_mask(uint8_t mask)
{
    led_mask = mask & 0x07;
    gpio_write(LED_RED,   (led_mask & BIT(0)) ? 0 : 1);
    gpio_write(LED_GREEN, (led_mask & BIT(1)) ? 0 : 1);
    gpio_write(LED_BLUE,  (led_mask & BIT(2)) ? 0 : 1);
}

_attribute_ram_code_ uint8_t get_led_mask(void)
{
    return led_mask;
}
