...
static void draw_watch_digit(OBDISP *display, int x, int y, uint8_t digit)
{
    ...
    if (mask & 0x01) obdRectangle(display, x + 5, y,      x + 25, y + 4,  1, 1);
    if (mask & 0x02) obdRectangle(display, x + 26, y + 4,  x + 30, y + 27, 1, 1);
    if (mask & 0x04) obdRectangle(display, x + 26, y + 34, x + 30, y + 57, 1, 1);
    if (mask & 0x08) obdRectangle(display, x + 5, y + 57, x + 25, y + 61, 1, 1);
    if (mask & 0x10) obdRectangle(display, x,     y + 34, x + 4,  y + 57, 1, 1);
    if (mask & 0x20) obdRectangle(display, x,     y + 4,  x + 4,  y + 27, 1, 1);
    if (mask & 0x40) obdRectangle(display, x + 5, y + 28, x + 25, y + 33, 1, 1);
}
...
_attribute_ram_code_ void epd_display(struct date_time _time, uint16_t battery_mv, int16_t temperature, uint8_t full_or_partial)
{
    ...
    obdWriteStringCustom(&obd, (GFXfont *)&Dialog_plain_16,
                         205, 18, (char *)buff, 1);
    obdRectangle(&obd, 6, 23, 243, 24, 1, 1);

    // Enlarged seven-segment numerals use the space freed by the debug voltage.
    draw_watch_digit(&obd, 27, 29, _time.tm_hour / 10);
    draw_watch_digit(&obd, 65, 29, _time.tm_hour % 10);
    obdRectangle(&obd, 119, 46, 124, 51, 1, 1);
    obdRectangle(&obd, 119, 69, 124, 74, 1, 1);
    draw_watch_digit(&obd, 148, 29, _time.tm_min / 10);
    draw_watch_digit(&obd, 186, 29, _time.tm_min % 10);

    // Battery and temperature now share the full-width status row.
    obdRectangle(&obd, 6, 96, 243, 97, 1, 1);
    obdRectangle(&obd, 15, 103, 42, 118, 1, 0);
    obdRectangle(&obd, 12, 107, 15, 114, 1, 1);
    battery_width = (battery_level * 24) / 100;
    if (battery_width)
        obdRectangle(&obd, 17, 105, 16 + battery_width, 116, 1, 1);
